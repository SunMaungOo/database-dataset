...
from config import DB_TYPE,MSSQL_GET_INFO_SQL
...
def get_dataset(connection_str:str,\
    ...
    if DB_TYPE=="mssql":
        return get_mssql_dataset(connection_str=connection_str,\
                                 host=host,\
                                database=database)

    return None
    
def get_mssql_dataset(connection_str:str,\
                      host:str,\
                      database:str)->Optional[List[Dataset]]:
